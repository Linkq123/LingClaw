...
- **Agent** (decision layer): `AgentPhase` enum (`Analyze`/`Act`/`Observe`/`Finish`), `AgentLoopCtx` state tracker, `ToolResultEntry`, `ObservationSummary`, `summarize_observations()`, `build_observation_context_hint()`, `maybe_annotate_observation()`, `is_finish()`, `is_empty_finish()` in `src/agent.rs`. The agent loop in `src/main.rs` uses `match react_ctx.phase()` to drive each iteration — one phase per arm. Inter-phase data flows via local variables (`pending_tool_calls`, `collected_results`, `last_observation_hint`). Phase transitions use `debug_assert!` for invariant enforcement. Observation summaries are non-destructive: raw tool results are always persisted in session history untouched; summaries go to separate WS events and system prompt hints.
