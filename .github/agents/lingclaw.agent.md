...
- `src/prompts.rs` — Session prompt init/load logic, bootstrap vs normal prompt flow with per-session bootstrap baselines, template discovery, daily memory log management, local-time snapshot helpers, avatar parsing (~837 lines)
