...
- **Config change** → Update `JsonConfig` / `JsonSettings` structs + `Config::load()` + `run_setup_wizard()` in `src/cli.rs` if interactive. Add env var override in the `Config::load()` env-var block if the setting needs runtime override.
