...
Runtime 检测到 SQLite I/O、完整性或约束故障后，会进入本次进程内不可逆的保护模式。服务端先退休被取消 direct run 的精确 live replay，再取消活动 Agent/Group run，因此已死亡的 run 不会在重连后恢复成伪活动状态，也不会误删更新连接的 replay。该流程不会伪装成用户 `/stop`。页面仍可读取 Session、历史、Usage 和独立配置文件，但会禁用发送消息、上传、Session/Group/Todo 等数据库写操作。修复磁盘空间、权限或数据库问题后重启 LingClaw；界面不会展示原始 SQL 错误。
...
历史回放会按服务器记录的运行边界保留同一轮的全部过程：中间回复后继续调用工具，或运行中补充用户指令，都不会拆出另一张执行栈。刷新、重连和加载更早消息时也遵守这个边界；相邻运行保持分开，缺少终态记录的旧过程仍显示为未完成。

Plan 的恢复目标即使位于尚未加载的更早消息中，点击执行栈的继续或修订入口也会按需加载并聚焦准确版本，不需要先手动加载历史。已丢弃的旧 Plan 在后续对话、刷新和重连后仍保持丢弃状态，不会再次要求回答问题；分页加载会保留已有的手动展开、折叠和当前修订草稿。

模型鉴权、限流、服务不可用、请求配置及上下文预算等终端失败会保留安全的错误类别。刷新或重启后仍能通过“查看错误”打开具体说明；需要检查配置时可直接进入模型设置。说明使用固定文案，不保存原始 Provider 响应或凭据。升级前缺少类别记录的失败仍显示通用提示。

用户消息右对齐，Assistant Markdown 使用自然排版。每轮动态过程聚合为一个 Execution Stack：

- Reasoning
- Tool call/result
- 执行提纲（可选的运行期软指导，与正式 Plan 明确区分）
- Sub-agent
- Orchestration

运行时默认展开；只有后端明确报告完成的运行才会自动折叠。失败、受阻、等待用户、停止、未完成和部分完成会保持展开，并持续显示每次点击都会重新定位的恢复入口；即使运行尚未显示任何 Tool/Reasoning/ReAct 步骤，停止或失败等 attention 终态仍会生成一个可恢复摘要，而无步骤成功不会产生空卡。用户手动展开或折叠始终优先。`system`/`progress` 提示不会提前结束运行；命令、Plan action 或预检错误也只显示错误卡，只有后端明确标记为顶层运行终态、且服务器连接身份与当前本地 run 同时匹配的错误或 `done` 才会解除运行状态。页面/TUI 会在每次建立 WebSocket 前重新协商 execution identity 协议，不会沿用上一个连接的旧结论；WebUI 的首次特性读取与首个连接共用同一连接意图，不会在切换 Session/Group 后迟到创建第二条 socket。协商同时限制响应头和 JSON body 等待时间，并会在新连接意图、取消或切换目标时主动取消旧请求。旧 daemon 仅支持首个连接内的兼容事件，连接断开或切换 Session/Group 时会显示刷新客户端或重启 daemon 的提示，不会冒险自动重连。若严格协议的 daemon 发来缺少运行身份的开始事件，客户端会关闭该连接、撤销“发送中”状态并给出恢复提示；已有过程栈会明确收口为可恢复的“未完成”，不会永久残留“运行中”。TUI 不会把草稿提前当成已保存，legacy 连接断开时会立即恢复尚未确认的文本、附件和 Plan mode；协商热开启 Groups 时会先建立 socket，再在后台恢复列表，短暂失败会有限自动重试。关闭后重新开启或切换目标会创建新的刷新周期，旧请求即使稍后返回，也不会覆盖新列表或消耗新周期的重试；重复的开启状态不会重复刷新。重连/历史重置后的无身份或旧身份 `done` 不会创建幽灵栈，也不会结束刚开始的新 run；其中与 run 无关的 Usage 总数仍会更新。存储保护只收口实际活动的 direct run，不会因为 Plan action、Session/Group 切换或 Group busy 而生成幽灵执行栈。缺少终态事实的历史过程会保守标记为未完成。运行标题会同步表达当前动作与对象、进度、失败点、验证、产物操作和未解决项；ReAct 阶段状态并入同一个运行壳，不再作为独立聊天卡。Tools 仍按过程步骤过滤；Reasoning 改为可持久化的“摘要 / 标准 / 详细”三档，摘要和标准只呈现派生轨迹信息，只有详细档会把原始 thinking 写入 DOM。Auto Debug 位于对话时间线之外的独立诊断面板。

新运行的终态会随 Session 一起写入 SQLite，包括稳定 run ID、完成/失败/停止等状态、原因、耗时、消息边界和关联 Plan。刷新、重连或重启 daemon 后，执行栈会从这条服务器事实恢复，不再把刚完成的工作统一显示为“未完成”；升级前没有这类记录的旧历史仍保守降级。模型瞬态重试只在当前栈中短暂显示 attempt，成功后消失；最终 Provider 错误只保留一份可见摘要。相同 Tool 与同一规范目标在后续重试成功时，旧失败详情仍可查看，但不再计入未解决项。服务器确认 Plan 已丢弃后，关联栈显示中性的“Plan 已丢弃”，问题表单与恢复入口同步消失。切换中英文会原位重译已完成栈的状态与恢复按钮。Auto Debug 是位于最新消息与输入区之间的可关闭 dock，不覆盖 Composer；按 `Escape` 也可关闭。

下一轮对话更新系统时间、目标或观察摘要不会清除已有运行结果。未恢复的工具或子任务失败会显示“部分完成”并保持展开；同一调用成功重试后才可正常完成，编排进度按真实子任务计数。等待回答的 Plan 在本轮结束后仍可丢弃，关联栈与问题表单会一起收口。触发安全轮次上限时，计划会退出忙态并保留已有进度：已批准执行可继续、修订或丢弃，未批准规划可修订或丢弃；刷新或重启后仍可恢复。

工具步骤点击后打开 Tool Inspector。桌面宽屏停靠在右侧，中等宽度使用浮动抽屉，手机使用底部面板。图片结果在 Inspector 内以懒加载画廊展示。

### 输入区

- 连接中、当前会话与历史尚未准备好或已离线时，发送按钮保持禁用，附近会显示简短状态。连接失败后可在同一页面点击“重新连接”；重连到同一 Session 会保留未发送文本与仍有效的附件。恢复后需自行点击发送或按 Enter，重连不会自动排队或重发草稿。模型无关命令也必须等待连接就绪。
...
- Agent 运行中发送普通文本会排队，在下一个 Analyze 阶段前注入；停止按钮和 `/stop` 会立即请求取消，包括 Finish 验证后的 hook、Usage 与终态候选准备。服务器在启动唯一的 Session/Plan/outcome 事务前做最后一次精确 Stop 仲裁：若停止先胜出，只会提交一个真实 `stopped`，不会随后又显示自然完成或合同失败；若自然终态事务已经被选中，服务器会等待它明确完成后再显示结果，不会通过取消数据库 future 另写相互矛盾的停止状态。终态提交会在持久化门禁内合并最新 Session，只接管本次运行的消息尾和 Plan 状态，因此同时保存的 Todos、模型/Effort 或工作目录选择不会被旧快照覆盖；若下一条用户消息已进入边界则安全拒绝旧终态写入。
- 模型不可用时，输入框显示原因并禁用普通发送；连接和会话就绪后，状态、帮助和设置类命令仍可使用。
...
Usage 页面展示今日与累计 Token、模型角色拆分、每日趋势和 Provider 趋势。角色包括 Primary、Fast、Sub-Agent、Memory、Reflection 和 Context。Memory/Reflection 的后台 Provider 调用会在成功后事务化写入 SQLite；Memory 会先记录 Usage，再解析提取结果和保存私有记忆，因此无效响应或文件保存失败不会漏记已经发生的 Provider 用量。Session 即使暂未加载也能完成计数，若 Session 已被删除则安全丢弃该迟到增量而不会误触发存储保护。删除 Session、关闭功能、进入存储保护或优雅退出时，LingClaw 会停止接受并等待对应后台生命周期；已开始的私有写入会被等待完成，尚未开始的写入会被取消，所以旧任务不会在删除后重建 Session Home。全零数据使用一个空状态；只有部分模块无数据时，仅对应区域显示空状态。
...
- **Plan Mode**：在当前 Session 内运行 `planning → needs_input → ready → executing → completed/failed/stopped/discarded` 状态机。正式 Plan 卡持续展示目标、revision、摘要与步骤；假设、风险、验收标准、验证方式和服务端完成检查收进渐进披露的“执行合同详情”。
...
- **Reasoning 密度**：摘要（默认）、标准和详细只改变界面呈现，不改变 Provider 返回或 Agent 行为。偏好保存在本地并跨刷新恢复；摘要和标准只显示字符数、段落数等派生信息，只有“详细”会把原始 reasoning trace 放入文档。
