...
如果配置在其他位置被修改，保存会报告冲突（HTTP 409）并保留当前草稿，暂时禁用再次保存。General、Agents 等配置页面在桌面和移动端都提供同一个“重新加载最新配置”按钮，可通过键盘访问；主动重新加载后会以最新配置替换草稿，之后可以继续修改并保存。离开 Console 时，未保存更改仍需要确认。
