...
- `~/.lingclaw/lingclaw.db` is the sole persistent source for Sessions, messages, Todos, Usage, Sub-agent snapshots, Groups, members, votes, messages, runs, and working-directory bindings. `.lingclaw.json`, MCP authentication, Structured Memory/Markdown, Skills, Agents, and private Session Homes remain filesystem data. Production code must never resume JSON dual-writes after migration.
