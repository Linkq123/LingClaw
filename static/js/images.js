...
export function initImageListeners() {
  if (dom.attachBtn) dom.attachBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleAttachPopup(); });
  if (dom.attachUrlBtn) dom.attachUrlBtn.addEventListener('click', () => {
  ...
  if (dom.attachLocalBtn) dom.attachLocalBtn.addEventListener('click', () => {
    if (dom.imageFileInput) dom.imageFileInput.click();
  });
  if (dom.imageUrlAddBtn) dom.imageUrlAddBtn.addEventListener('click', () => {
    if (dom.imageUrlField) { addImageUrl(dom.imageUrlField.value); dom.imageUrlField.value = ''; }
  });
  if (dom.imageUrlField) dom.imageUrlField.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); addImageUrl(dom.imageUrlField.value); dom.imageUrlField.value = ''; }
    if (e.key === 'Escape') closeAttachPopup();
  });
  if (dom.imageFileInput) dom.imageFileInput.addEventListener('change', () => {
